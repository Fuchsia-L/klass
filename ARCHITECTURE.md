...
- `getSyncScheduler(options?: SyncSchedulerOptions): SyncScheduler` — returns the singleton instance; the first call must supply options, later calls ignore them. `resetSyncSchedulerForTests(): void` disposes the singleton (intended for tests only). Callers: future app wiring (Phase 5+) for token-saved / AppState-active triggers.
